...
require_contains scripts/ci/backup_restore_check.py "TINYZKP_BACKUP_TEST_DATE" "backup/restore check preserves unprivileged deterministic backup timestamp testing"
require_contains scripts/ci/backup_restore_check.py "TINYZKP_BACKUP_TEST_REMOTE_DATE" "backup/restore check preserves unprivileged deterministic remote date testing"
...
require_contains docs/operations.md "/var/lib/tinyzkp-runtime/billing-venv" "operations guide documents external host billing virtualenv"
...
require_contains docs/runbooks/2026-06-23-reconciliation-deploy.md "retired" "obsolete reconciliation runbook is an explicit tombstone"
require_contains docs/runbooks/2026-06-23-reconciliation-deploy.md "docs/operations.md" "reconciliation tombstone points to the current operator procedure"
require_no_regex '```' "reconciliation tombstone contains no executable code blocks" docs/runbooks/2026-06-23-reconciliation-deploy.md
...
require_contains deploy/hetzner/deploy.sh "/var/lib/tinyzkp-runtime/billing-venv/bin/gunicorn" "Hetzner deploy writes external venv-backed billing webhook unit"
...
require_contains deploy/hetzner/setup.sh "/var/lib/tinyzkp-runtime/billing-venv/bin/gunicorn" "Hetzner setup runs billing webhook from external virtualenv"
