...
def check_billing_and_release() -> None:
    ...
    require(
        release["status"] == "blocked", "backend v1 must remain blocked during recovery"
    )
    for gate_name in (
        ...
        require(
            not release["gates"][gate_name]["passed"],
            f"unearned release gate is marked passed: {gate_name}",
        )
