...
1. Create a GitHub OAuth app with callback `https://api.tinyzkp.com/v1/auth/github/callback` and record the client ID and secret in the owner-only API environment file.
...
- Copy the tracked environment examples to `/etc/tinyzkp/beta` and `/etc/tinyzkp/worker`; set files to `0600` and directories to `0700`.
...
1. Build and publish digest-pinned API, worker, and PostgreSQL images from the exact candidate SHA.
...
- Run the fixed-host 1M/16M matrix, customer_cubic8 matrix, fault/fuzz suite, security review, four-job load test, and identity check on the final candidate.
...
- Run the 24-hour allowlisted canary and validate its evidence with `validate_public_beta_canary.py`.

## Activation

1. Populate `release/public-beta-evidence.json` with SHA-256-bound artifacts and run `public_beta_gate.py` against the exact candidate SHA.
2. Tag the candidate `public-beta-v*`. The release workflow builds OCI archives, SPDX SBOMs, checksums, attestations, and the signed public-beta authorization.
