...
A Claude Skill that teaches Claude when and how to use the **TinyZKP** MCP server to mint and verify transparent state-transition receipts.
...
1. Recognize that the request maps to a state-transition receipt
...
A hosted STARK receipt service for verifiable state transitions. Transparent (no trusted setup), post-quantum (hash-based). MIT-licensed open-source backend at https://github.com/logannye/hc-stark.
