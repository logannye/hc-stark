(function () {
  ...
  function reviewedCheckout(config, release, cadence) {
    if (!config || config.schema_version !== 1 || config.checkout_enabled !== true) return null;
    if (config.launch_gate_status !== "passed" || config.provider !== "lemon_squeezy") return null;
    if (!release || release.schema_version !== 1 || release.status !== "qualified") return null;
