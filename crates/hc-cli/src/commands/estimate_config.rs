...
/// `hc_wasm::estimate_request`. That function is the single implementation
/// of the cost model — the hosted WASM API (`hc-wasm`'s `estimate_json`)
/// calls the exact same function, so the CLI and the API cannot diverge on
/// a number the way Phase 1a's `conventional` estimate did (computed two
/// ways, 7.8x apart). Nothing here recomputes any part of that mapping.
///
...
pub fn run(config_path: &Path) -> Result<EstimateResponseV1> {
    ...
    hc_wasm::estimate_request(request)
