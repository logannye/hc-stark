...
impl HcMcpServer {
    ...
    #[rmcp::tool(description = "List all available proof templates with IDs, summaries, and tags. Use this to discover what kinds of proofs you can generate.")]
    async fn list_templates(&self) -> Result<CallToolResult, ErrorData> {
        self.list_templates_impl().await
    }

    #[rmcp::tool(description = "List all registered workload IDs. Workloads are predefined proof programs.")]
    async fn list_workloads(&self) -> Result<CallToolResult, ErrorData> {
        self.list_workloads_impl().await
    }

    #[rmcp::tool(description = "Get full parameter schema and example JSON for a specific proof template. Call this before prove_template to understand what parameters are needed.")]
    ...
    #[rmcp::tool(description = "Get server capabilities, version, and recommended workflow. Start here if you're unsure what this server can do.")]
    async fn get_capabilities(&self) -> Result<CallToolResult, ErrorData> {
        self.get_capabilities_impl().await
    }

    #[rmcp::tool(description = "Generate a zero-knowledge proof from a template ID and parameters. Returns a job_id — call poll_job to check progress.")]
    ...
    #[rmcp::tool(description = "Generate a proof from a registered workload ID. Returns a job_id — call poll_job to check progress.")]
    ...
    #[rmcp::tool(description = "Check the status of a proof job. Returns: pending, running, succeeded, or failed.")]
    async fn poll_job(
        &self,
    ...
    #[rmcp::tool(description = "Verify a proof independently. Pass the base64-encoded proof from get_proof. Returns {valid: true/false}.")]
    ...
    #[rmcp::tool(description = "Retrieve the base64-encoded proof bytes for a completed job. Pass the result to verify_proof for independent verification.")]
    async fn get_proof(
        &self,
    ...
    #[rmcp::tool(description = "Get a human-readable summary of what a proof job attests to, including template, public inputs, and status.")]
